...
class OrderFlowConfig:
    """
    Configuration for Poisson order flow simulator
    ...
    lambda_market: float
        intensity of market order arrivals
    lambda_cancel: float
        intensity of cancellation events
    mid_price: float
        Initial mid prace
    tick_size:
        Minimum price increment
    max_depth_ticks:
        Maximum distance from mid placement of new limit orders
    mean_size: float
        Mean order size
    seed: Optional[int]
    ...
    lambda_limit: float = 5.0
    lambda_market: float = 2.0
    lambda_cancel: float = 2.0
    mid_price: float = 100.0
    tick_size: float = 0.01
    max_depth_ticks: float = 50
...
class SimulationEvent:
    """
    Single event emitted by the simulator.
    ...
    side: Optional[Side]
        BUY or SELL where applicable
    order_id: Optional[int]
        Relevant order id for cancel or created order
    trades: list[Trade]
        Any trades generated by this event.
